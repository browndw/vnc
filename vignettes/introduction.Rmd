...
The purpose of [Variability-Based Neighbor Clustering](https://doi.org/10.1093/oxfordhb/9780199922765.013.0014) is to divide the use of a word or phrase into historical periods based on changes in frequency. Rather than assuming that a year, decade, or other division is statistically meaningful, the algorithm clusters segments of time into periods.

For this to work, the time data that we input must be evenly spaced. There can't be any gaps. In the data above, it is clear that there are many missing decades. If we tried to plot the data, a function called **is_sequence()** would halt the process and produce an error.

```{r check_seq, error=TRUE}
vnc_scree(witch_hunt$decade, witch_hunt$counts_permil, distance_measure = "sd")
```
...
vnc_scree(wh_d$decade, wh_d$counts_permil, distance_measure = "sd")
```
...
hc <- vnc_clust(wh_d$decade, wh_d$counts_permil, distance_measure = "sd")
```
...
clust_df <- data.frame(decade=as.numeric(names(unlist(cut_hc))),
  clust=rep(c(paste0("clust_", seq_along(cut_hc))),
...
Next, we'll convert our cluster data into dendrogram data using **as.dendrogram( )** from [**ggdendro**](https://cran.r-project.org/package=ggdendro). We also MUST maintain the order of our time series. There are a variety of ways of doing this, but [**dendextend**](https://cran.r-project.org/package=dendextend) has an easy function called **sort( )**. We'll take the easy way!
