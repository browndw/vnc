...
#' @param distance.measure Indicating whether the standard deviation or
...
vnc_clust <- function(time, values, distance.measure = c("sd", "cv")) {
  distance.measure <- match.arg(distance.measure)

  if (!is.sequence(time)) stop ("It appears that your time series contains gaps or is not evenly spaced.")
  if (length(time) != length(values)) stop ("Your time and values vectors must be the same length.")
